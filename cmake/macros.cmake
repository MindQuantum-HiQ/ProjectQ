...
# Automatically define a CMake imported target based on the value of existing CMake variables
#
# define_target(<namespace> <comp> <incpath>)
macro(define_target namespace comp incpath)
  string(TOUPPER ${comp} uppercomponent)
  add_library(${namespace}::${comp} UNKNOWN IMPORTED)
  if(incpath)
    set_target_properties(${namespace}::${COMPONENT} PROPERTIES INTERFACE_INCLUDE_DIRECTORIES "${incpath}")
  endif()
  if(EXISTS "${${namespace}_${uppercomponent}_LIBRARY}")
    set_target_properties(
      ${namespace}::${comp} PROPERTIES IMPORTED_LINK_INTERFACE_LANGUAGES "CXX"
  ...
  if(EXISTS "${${namespace}_${uppercomponent}_LIBRARY_RELEASE}")
    ...
    set_target_properties(
      ${namespace}::${comp} PROPERTIES IMPORTED_LINK_INTERFACE_LANGUAGES_RELEASE "CXX"
                                       IMPORTED_LOCATION_RELEASE "${${namespace}_${uppercomponent}_LIBRARY_RELEASE}")
  endif()
  if(EXISTS "${${namespace}_${uppercomponent}_LIBRARY_DEBUG}")
    ...
    set_target_properties(
      ${namespace}::${comp} PROPERTIES IMPORTED_LINK_INTERFACE_LANGUAGES_DEBUG "CXX"
                                       IMPORTED_LOCATION_DEBUG "${${namespace}_${uppercomponent}_LIBRARY_DEBUG}")
  endif()
  ...
  if(${namespace}_${uppercomponent}_DEPENDENCIES)
    ...
    foreach(dep ${${namespace}_${uppercomponent}_DEPENDENCIES})
      list(APPEND ${namespace}_${uppercomponent}_TARGET_DEPENDENCIES Boost::${dep})
    endforeach()
    if(COMPONENT STREQUAL "thread")
      list(APPEND ${namespace}_${uppercomponent}_TARGET_DEPENDENCIES Threads::Threads)
    endif()
    # cmake-lint: disable=C0307
    set_target_properties(${namespace}::${comp} PROPERTIES INTERFACE_LINK_LIBRARIES
                                                           "${${namespace}_${uppercomponent}_TARGET_DEPENDENCIES}")
  endif()
endmacro(define_target)

# ==============================================================================
# Automatically extract the test names from C++ files written with Boost::Test

set(_boost_test_list)
# Define and keep track of Boost::Test executable
#
# add_boost_test(<source_file> [<library> [<library> ...]])
#
# Define a new Boost::Test executable and add the target name to the _boost_test_list variable
macro(add_boost_test source_file_name)
  get_filename_component(TEST_EXECUTABLE_NAME ${source_file_name} NAME_WE)

  add_executable(${TEST_EXECUTABLE_NAME} ${source_file_name})
  target_link_libraries(${TEST_EXECUTABLE_NAME} PRIVATE ${ARGN} Boost::unit_test_framework)
  ...
  foreach(hit ${FOUND_TESTS})
    ...
    add_test(NAME "${TEST_EXECUTABLE_NAME}.${TEST_NAME}" COMMAND ${TEST_EXECUTABLE_NAME} --run_test=${TEST_NAME}
