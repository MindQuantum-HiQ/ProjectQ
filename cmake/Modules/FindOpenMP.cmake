...
# in Fortran, an implementation may provide an omp_lib.h header or omp_lib module, or both (OpenMP standard, section
# 3.1) Furthmore !$ is the Fortran equivalent of #ifdef _OPENMP (OpenMP standard, 2.2.2) Without the conditional
# compilation, some compilers (e.g. PGI) might compile OpenMP code while not actually enabling OpenMP, building code
# sequentially
...
\n${OpenMP_TRY_COMPILE_OUTPUT}\n\n")
        ...
        foreach(_OPENMP_IMPLICIT_LIB IN LISTS OpenMP_${LANG}_IMPLICIT_LIBRARIES)
          ...
          string(REGEX REPLACE "([][+.*?()^$])" "\\\\\\1" _OPENMP_IMPLICIT_LIB_PLAIN_ESC
                               "${_OPENMP_IMPLICIT_LIB_PLAIN}")
          string(REGEX REPLACE "([][+.*?()^$])" "\\\\\\1" _OPENMP_IMPLICIT_LIB_PATH_ESC "${_OPENMP_IMPLICIT_LIB}")
          if(NOT
             ("${_OPENMP_IMPLICIT_LIB}" IN_LIST CMAKE_${LANG}_IMPLICIT_LINK_LIBRARIES
              OR "${CMAKE_${LANG}_STANDARD_LIBRARIES}" MATCHES
                 "(^| )(-Wl,)?(-l)?(${_OPENMP_IMPLICIT_LIB_PLAIN_ESC}|${_OPENMP_IMPLICIT_LIB_PATH_ESC})( |$)"
              OR "${CMAKE_${LANG}_LINK_EXECUTABLE}" MATCHES
                 "(^| )(-Wl,)?(-l)?(${_OPENMP_IMPLICIT_LIB_PLAIN_ESC}|${_OPENMP_IMPLICIT_LIB_PATH_ESC})( |$)"))
            if(_OPENMP_IMPLICIT_LIB_DIR)
              set(OpenMP_${_OPENMP_IMPLICIT_LIB_PLAIN}_LIBRARY
                  "${_OPENMP_IMPLICIT_LIB}"
                  CACHE FILEPATH "Path to the ${_OPENMP_IMPLICIT_LIB_PLAIN} library for OpenMP")
            else()
              find_library(
        ...
        # We do not know how to extract implicit OpenMP libraries for this compiler. Assume that it handles them
        # automatically, e.g. the Intel Compiler on Windows should put the dependency in its object files.
        set("${OPENMP_LIB_NAMES_VAR}"
            ""
            PARENT_SCOPE)
      endif()
  ...
  if(CMAKE_${LANG}_COMPILER_LOADED)
    ...
    if(OpenMP_${LANG}_FOUND)
      ...
      if(OpenMP_${LANG}_FLAGS)
        ...
        set_property(TARGET OpenMP::OpenMP_${LANG} PROPERTY INTERFACE_COMPILE_OPTIONS
                                                            "$<$<COMPILE_LANGUAGE:${LANG}>:${_OpenMP_${LANG}_OPTIONS}>")
        unset(_OpenMP_${LANG}_OPTIONS)
      endif()
      if(OpenMP_${LANG}_INCLUDE_DIRS)
        set_property(TARGET OpenMP::OpenMP_${LANG} PROPERTY INTERFACE_INCLUDE_DIRECTORIES
                                                            "$<BUILD_INTERFACE:${OpenMP_${LANG}_INCLUDE_DIRS}>")
      endif()
      if(OpenMP_${LANG}_LIBRARIES)
        set_property(TARGET OpenMP::OpenMP_${LANG} PROPERTY INTERFACE_LINK_LIBRARIES "${OpenMP_${LANG}_LIBRARIES}")
      endif()
    endif()
  endif()
endforeach()
