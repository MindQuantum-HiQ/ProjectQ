...
@has_pyparsing
def test_read_qasm_custom_gate_with_param(eng):
    qasm_str = '''
OPENQASM 2.0;
include "qelib1.inc";
...
'''
    ...
    assert len(bits_map['c']) == 3
    assert len(filter_gates(eng.backend, AllocateQubitGate)) == 3
    assert len(exclude_gates(eng.backend, AllocateQubitGate)) == 2
...
'''
    ...
    assert len(bits_map['c']) == 3
    assert len(filter_gates(eng.backend, AllocateQubitGate)) == 3
    assert len(exclude_gates(eng.backend, AllocateQubitGate)) == 1


@has_pyparsing
def test_read_qasm_init_error(eng):
    qasm_str = '''
OPENQASM 3.0;
creg q = 0;
'''
...
@has_pyparsing
def test_read_qasm_custom_gate_error(eng):
    qasm_str = '''
OPENQASM 2.0;
gate mygate(alpha) a {
rx(alpha) a;
