...
def test_main_engine_flush():
    ...
    # Simulate adding a qubit manually (used in the case of parametric gates)
    weak_qubit = WeakQubitRef(eng, idx=10)
    eng.active_qubits.add(weak_qubit)
    eng._qubit_idx = 11
    ...
    assert backend.received_commands[4].gate == DeallocateQubitGate()
