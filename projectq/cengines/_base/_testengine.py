...
class CompareEngine(BasicEngine):
    ...
    def is_available(self, cmd):  # pylint: disable=unused-argument,no-self-use
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, CompareEngine) or len(self._l) != len(other._l):
            return False
        for i, cmds in enumerate(self._l):
            if len(cmds) != len(other._l[i]):
                return False
            for j, cmd in enumerate(cmds):
                if not _compare_cmds(cmd, other._l[i][j]):
                    return False
        return True
    ...
    def __str__(self):
        ...
        for qubit_id in range(len(self._l)):
            lines.append(
                ''.join(["Qubit {0} : ".format(qubit_id), ', '.join(str(command) for command in self._l[qubit_id])])
            )
        return '{}\n'.format('\n'.join(lines))
