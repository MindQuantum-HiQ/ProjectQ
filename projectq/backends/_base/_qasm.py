...
class OpenQASMBackend(BasicEngine):  # pylint: disable=too-many-instance-attributes
    ...
    def is_available(self, cmd):
        ...
        """
        if has_negative_control(cmd) or cmd.gate.is_parametric():
            return False
        ...
        if n_controls == 0:
            if gate in (H, S, Sdag, T, Tdag, X, NOT, Y, Z, Swap):
                is_available = True
            if isinstance(gate, (PhNum, RNum, RxNum, RyNum, RzNum)):
                is_available = True
        elif n_controls == 1:
    ...
    def _store(self, cmd):  # pylint: disable=too-many-branches,too-many-statements
        ...
        if gate == Allocate:
            ...
            elif isinstance(gate, (RNum, RxNum, RyNum, RzNum)):
                self._output.append(
